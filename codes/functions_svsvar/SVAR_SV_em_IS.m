function [ Param_est, loglike, count, VarTheta, SAnalysis] = SVAR_SV_em_IS( y, specs)
% Bertsche, Braun (2020): Identification of Structural Vector
% Autoregressions by Stochastic Volatility
% Main File that implements 
% 1) Runs EM-1 Algorithm to get starting values
% 2) Iterates EM-2 algorithm
...
%% EM-1 algorithm is used to get starting values
specs.param_init = [];  
[Param_init] = SVAR_SV_em_approx( y, specs);
A_it = Param_init.A;
vA_it = A_it(:);
B_it = Param_init.B;
vB_it = pinv(S_b)*B_it(:);
...
%% EM 2 Algorithm 
nreps = [ceil(specs.M2/4), ceil(specs.M2/2), specs.M2];
tols = [ specs.tol*4 , specs.tol*2, specs.tol ]; 
nrep = nreps(1); tol = tols(1);
idx_n = 1; 
CRNnorm = randn(T,nrep,K); 
llh_diff = inf;
count = 0;     
...
while and(llh_diff>tol,count<max_it) 
    ...
    dQ1s = zeros(K,1);
    for j=idx_r
        [ Ht(:,j),VarHt(:,j),CovHtHtm1(:,j),iV(:,j) ] = e_step_IS( e_it(:,j), phi_it(j), s_it(j), Ht(:,j), CRNnorm(:,:,j) , scale);
    ...
    %% Stepwise increase the number of IS replications
    if or(llh_diff<tol,count>idx_n*ceil(max_it/length(nreps)))
        if nrep==nreps(end)
        else
            idx_n = idx_n + 1;
            nrep = nreps(idx_n);
...
%% Arbitrary ordering, here to maximize the trace
...
%% EVALUATE likelihood and likelihood criteria
if nargout > 1  
    n_param = length(s_it) + length(phi_it) + length(vB_it) + length(vec(A_it)); 
    [llh, intlikestd, pval_fv] = llh_IS( u_it, B_it, M, phi_it, s_it, Ht); %
    [llh2, intlikestd2, pval_fv2] = llh_IS( Param_init.u, Param_init.B, M, Param_init.phi, Param_init.s, Param_init.H);
    
    loglike.llh = llh; % log likelihood value 
    loglike.llh_sdv = intlikestd; % standard deviation estimate
    loglike.pval_fv = pval_fv;  % sp-value test for finite IS weights 
    loglike.llhEM1 = llh2;  % log likelihood value  (EM1)
    loglike.llh_sdvEM1 = intlikestd2; % standard deviation estimate (EM1)
    loglike.pval_fv_em1 = pval_fv2; % sp-value test for finite IS weights (EM1)
    loglike.aic = -2*llh + 2*n_param; % AIC information criteria
    loglike.bic = -2*llh + log(T)*n_param; % BIC information criteria
end  
%% IF REQUESTED: COMPUTE STANDARD Errors by either Oakes or Louis Method
% Oakes method uses common random numbers
if nargout>3  
    %% Do Oakes Method based on Common Random numbers 
    nA = K*(K*p+1); 
    paramhat = [phi_it(idx_r); s_it(idx_r); vec(A_it'); S_b'*vec(B_it)];
    Qd2 = Expected_Hess(Param_est,Y,X, Ht,VarHt,CovHtHtm1,iV,S_b);
    objfun_g = @(x)Expected_Grad(x,Param_est,Y,X,p, Ht,S_b,  scale, CRNnorm);  
    Qd1d2 = Gradp(objfun_g,paramhat);
    HessianOakesFormula = Qd2 + Qd1d2; 
